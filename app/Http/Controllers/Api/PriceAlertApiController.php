...
class PriceAlertApiController extends Controller
{
    use CrudTrait;
    protected $viewVariable = 'price_alert';
    protected $relationKey = 'price_alert_id';

    public function model() {
        $data = ['limit' => -1, 'model' => PriceAlert::class, 'sort' => ['id','desc']];
        return $data;
    }
    ...
    public function sendEmail(Request $request) {
        ...
        $success = false;
        if($id) {
            $data = PriceAlert::where('id', $id)->first();
    ...
    public function sendMultiEmail(Request $request) {
        ...
        if(isset($request->id)) {
            $ids = $request->id;
            $data = PriceAlert::whereIn('id',$ids)->get();
